...
if (isset($_POST['book'])) {
    if (!$is_logged_in) {
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            $insert_query = "INSERT INTO visit_booking (patient_id, doctor_id, speciality_id, time_id, Note, appointment_date, status, patient_notified) VALUES ('$patient_id', '$doctor_id_post', '$speciality_id', '$time_id', '$note', '$appointment_date', 'accepted', 1)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['action']) && !isset($_POST['book'])) {
    ...
    $query = "SELECT u.id, u.firstname, u.lastname, u.phone_number, u.address, c.category_name,
        ...
        <div class="container">
                        ...
                        <div class="card-body p-3">
                            <div class="row align-items-center">
                                <div class="col-md-2 text-center">
                                    <?php
                                    $initials = strtoupper(substr((string)($doc['firstname'] ?? ''), 0, 1) . substr((string)($doc['lastname'] ?? ''), 0, 1));
                                    ...
                                    ?>
                                    <div class="doctor-avatar rounded-circle d-flex align-items-center justify-content-center mx-auto" style="overflow:hidden;">
                                        <?php if ($has_photo): ?>
                                            <img src="<?php echo htmlspecialchars($profile_image); ?>" alt="Dr. <?php echo htmlspecialchars((string)($doc['firstname'] ?? '') . ' ' . (string)($doc['lastname'] ?? '')); ?>" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:50%;">
                                        <?php else: ?>
            ...
            <?php elseif ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['book'])): ?>
                <div class="card text-center py-5 mt-4">
                    <div class="card-body"><i class="fas fa-user-doctor fs-1 text-warning mb-3"></i>
                        <h4 class="fw-bold">No Doctors Found</h4>
                    ...
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="patient_dashboard.php">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6"><label for="time_id" class="form-label fw-bold">Time</label><select name="time_id" id="time_id" class="form-select" required>
                                        <option value="">-- Select a Date --</option>
                                    </select></div>
                                <div class="col-md-6"><label for="speciality_id" class="form-label fw-bold">Reason</label><select name="speciality_id" id="speciality_id" class="form-select" required><?php $doctor_id_get = intval($_GET['doctor_id']);
                                                                                                                                                                                                        $spec_result = mysqli_query($conn, "SELECT s.id, s.doctor_speciality FROM speciality s INNER JOIN doctor_speciality ds ON ds.speciality_id = s.id WHERE ds.doctor_id='$doctor_id_get'");
...
<script>
    $(document).ready(function() {
        $('#appointment_date').on('change', function() {
            ...
            var doctorId = <?php echo isset($_GET['doctor_id']) ? intval($_GET['doctor_id']) : 0; ?>;
            if (selectedDate && doctorId > 0) {
                ...
                $.ajax({
                    ...
                    error: function() {
                        $('#time_id').html('<option value="">Error</option>');
                    }
                });
            } else {
                $('#time_id').html('<option value="">-- Select a Date --</option>');
            }
        });
