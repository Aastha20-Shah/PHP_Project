...
if ($action === 'upload') {
  ...
  if (!is_array($file)) {
    $_SESSION['doctor_photo_message'] = 'Please choose an image to upload.';
    $_SESSION['doctor_photo_message_type'] = 'warning';
    header("Location: $redirect");
    exit;
  }

  $result = medikit_doctor_profile_image_upload($conn, $doctor_id, $file);
  $_SESSION['doctor_photo_message'] = (string)($result['message'] ?? '');
  $_SESSION['doctor_photo_message_type'] = ($result['success'] ?? false) ? 'success' : 'danger';
} elseif ($action === 'delete') {
  $result = medikit_doctor_profile_image_delete($conn, $doctor_id);
  $_SESSION['doctor_photo_message'] = (string)($result['message'] ?? '');
  $_SESSION['doctor_photo_message_type'] = ($result['success'] ?? false) ? 'success' : 'danger';
}
